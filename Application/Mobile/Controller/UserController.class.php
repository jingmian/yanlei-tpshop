...
class UserController extends MobileBaseController
{
    ...
    */
    ...
    public function order_detail()
    {
        ...
        $my_id = $order_info['my_id'];
        $ltlist = M('user_my')->where('my_id ='.$my_id)->find();
        $measurements = json_decode(htmlspecialchars_decode($ltlist['measurements']),true);
        ...
        $this->assign('ltlist', $ltlist);
        $this->assign('measurements', $measurements);
        $this->display();
    }
    ...
    public function my()
    {
        $my_list = get_user_my_list($this->user_id);
        //dump($my_list);die;
        $this->assign('mylists',$my_list);
    ...
    //          //dump($my_lt);die;        
    //          $logic = new UsersLogic();
    //          $data = $logic->add_mylist($this->user_id, 0, $my_lt);
    //          if($data['status'] == 1){
    //             //$this->success($data['msg']);
    //             header("Location:" . U('Mobile/User/my'));
    //          }else{
    //             $this->error($data['msg']);
    //          }
    //     }
    ...
    public function edit_my(){
        ...
        $id = I('id');
        //dump($id);
        $my_list = M('user_my')->where(array('my_id' => $id, 'user_id' => $this->user_id))->find();
        //dump($my_list);die;
        $measurements = json_decode(htmlspecialchars_decode($my_list['measurements']),true);

        //dump($measurements);die;
        if (IS_POST) {       
             $logic = new UsersLogic();
             //dump($id);die;
             $data = $logic->add_mylist($this->user_id, $id, I('post.'));
             //dump($data);die;
             if($data['status'] == 1){
                header("Location:" . U('/Mobile/User/my'));
             }else{
    ...
    public function del_my(){
        $id = I('get.id');
        //dump($id);die;
        $my_list = M('user_my')->where("my_id = $id")->find();
        //dump($my_list);die;
        $row = M('user_my')->where(array('user_id'=>$this->user_id,'my_id'=>$id))->delete();
        //dump($row);die;                
        // 如果删除的是默认量体信息 则要把第一个量体信息设置为默认量体信息
        if($my_list['is_default'] == 1)
        {
            $my_list2 = M('user_my')->where("user_id = {$this->user_id}")->find();            
            $my_list2 && M('user_my')->where("my_id = {$my_list2['my_id']}")->save(array('is_default'=>1));
        }        
        // if(!$row)
        //     $this->error('操作失败',U('User/my'));
        // else
        //     $this->success("操作成功",U('User/my'));
        if ($row) {
            header("Location:" . U('Mobile/User/my'));
            exit;
        }
     ...
     * 设置默认量体数据
     */
    public function set_mydefault()
    {
        $id = I('get.id');
        //dump($id);die;
        M('user_my')->where(array('user_id' => $this->user_id))->save(array('is_default' => 0));
        $row = M('user_my')->where(array('user_id' => $this->user_id, 'my_id' => $id))->save(array('is_default' => 1));
        //dump($row);die;
        if ($row == 1) {
            header("Location:" . U('Mobile/User/my'));
            exit;
