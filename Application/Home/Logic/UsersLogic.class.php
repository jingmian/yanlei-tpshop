...
//        	}
    ...
    public function add_mylist($user_id,$my_id,$data){      
        $post = $data;
        //dump($post);die;
        ...
        if($my_id == 0)
        {
            $c = M('UserMy')->where("user_id = $user_id")->count();
            //dump($c);die;
            if($c >= 10)
                return array('status'=>-1,'msg'=>'最多只能添加10个量体信息','result'=>'');
        } 
        //编辑模式
        if($my_id > 0){
            $my = M('user_my')->where(array('my_id'=>$my_id,'user_id'=> $user_id))->find();
            //dump($post);
            $row = M('user_my')->where(array('my_id'=>$post['id'],'user_id'=> $post['user_id']))->save($post);
            //dump($row);die;
            if($row == 1){
                return array('status'=>1,'msg'=>'保存成功','result'=>'$my_id');
            }else{
               return array('status'=>-1,'msg'=>'保存失败','result'=>''); 
            }  
        }
        //添加模式
        $post['userId'] = $user_id;
        // 如果目前只有一个量体信息则改为默认量体信息
        $c = M('user_my')->where("user_id = {$post['userId']}")->count();        
        if($c == 0)  $post['is_default'] = 1;
        $my_id = M('user_my')->add($post);
        //如果设为默认量体信息
        $insert_id = M()->getLastInsID();
        $map['user_id'] = $user_id;
        $map['my_id'] = array('neq',$insert_id);
               
        if($post['is_default'] == 1)
            M('user_my')->where($map)->save(array('is_default'=>0));
        if(!$my_id)
            return array('code'=>-1,'msg'=>'添加失败','result'=>'');
        return array('code'=>0,'msg'=>'添加成功','result'=>$my_id);
    }
